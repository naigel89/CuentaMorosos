...
2. **Scroll + animation**: Hoist celebration trigger to `CuentaMorososApp`. On calculation apply → dismiss sheet, scroll to `SettlementPanel`, launch animation after 100ms delay. Scroll using `ScrollState.animateScrollTo()`.
...
|------|------------|------------|
| Scroll target item index changes with data | Low | Use stable profile ID-based scroll or section key, not hardcoded index |
