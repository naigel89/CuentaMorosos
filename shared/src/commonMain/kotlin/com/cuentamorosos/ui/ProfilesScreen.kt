    ...
    LazyColumn(
        ...
        if (profiles.isEmpty()) {
            ...
            itemsIndexed(sortedProfiles, key = { _, profile -> profile.id }) { index, profile ->
                ...
                // Saldar una deuda atenúa la tarjeta en lugar de cambiarla de golpe.
                val settledAlpha by animateFloatAsState(
                    targetValue = if (isSettled) 0.75f else 1f,
                ...
                ProfileCard(
                    ...
                    modifier = Modifier
                        // Reordenar la lista (al saldar, al renombrar) desliza las
                        // tarjetas a su nueva posición en vez de teletransportarlas.
                        .animateItemPlacement(NeoFintechMotion.placement)
                        .appearOnce(entrance, key = profile.id, index = index)
                        .graphicsLayer { alpha = settledAlpha },
                )
            }
        }
    }
